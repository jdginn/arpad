...
func (g *Generator) generatePreamble(buf *bytes.Buffer) {
	fmt.Fprintf(buf, "// Code generated by motuhttpgen. DO NOT EDIT.\n\n")
...
func main() {
	...
	flag.StringVar(&configPath, "config", "", "Path to MOTU api_spec.md")
	flag.StringVar(&outputPath, "output", "motu.go", "Output file path")
	flag.StringVar(&pkgName, "package", "motu", "Package name for generated code")
	flag.Parse()
	...
	rawActions, err := Read(file)
	if err != nil {
		log.Fatalf("Failed to read config: %v", err)
	}
	fmt.Printf("Raw actions: %+v\n\n\n", rawActions)
	parsedActions, err := Parse(rawActions)
	if err != nil {
		log.Fatalf("Failed to parse config: %v", err)
	}
	fmt.Printf("Parsed actions: %+v\n\n\n", parsedActions)
