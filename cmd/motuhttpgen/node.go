...
func BuildTree(actions []Action) *Node {
	root := &Node{
		pathSegment: "Motu", // Convention: top-level node
		Name:        "Motu",
		Fields:      []*Field{},
		// Parent: nil
	}
...
func insertPattern(root *Node, act Action) {
		...
		if seg.Kind == WildcardSegment {
			...
			if curr.Qualifier == nil {
				curr.Qualifier = &Qualifier{
					...
					ParamType: apiTypeToGoTypeLiteral(act.Arguments[nthWildcard].Type),
				}
			}
			nthWildcard++
	...
	curr.Endpoint = &Endpoint{
		ValueType:     apiTypeToGoTypeLiteral(act.Arguments[len(act.Arguments)-1].Type), // e.g. "float64"
